...
const UniversityDashboard = () => {
    ...
    useEffect(() => {
        const fetchUniversityDetails = async () => {
            ...
            if (token) {
                try {
                    const response = await axios.get('${import.meta.env.VITE_API_BASE_URL}/get-university-details', {
    ...
    const handleIssueCertificate = async () => {
        ...
        try {
            ...
            await axios.post('${import.meta.env.VITE_API_BASE_URL}/verify-signature', verificationFormData, {
            ...
            const uploadResponse = await axios.post('${import.meta.env.VITE_API_BASE_URL}/upload-certificate', uploadFormData, {
                headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'multipart/form-data' }
            });
            ...
            const hashResponse = await axios.post('${import.meta.env.VITE_API_BASE_URL}/prepare-certificate-hash', {
                ...formData,
                ipfsCid,
            }, { headers: { 'Authorization': `Bearer ${token}` } });
            ...
            const verificationUrl = `${window.location.origin}${import.meta.env.VITE_API_BASE_URL}/verify/result?qrData=${encodeURIComponent(rawQrData)}`;
    ...
    const sendToStudent = async () => {
        ...
        try {
            ...
            await axios.post('${import.meta.env.VITE_API_BASE_URL}/send-certificate-email', {
